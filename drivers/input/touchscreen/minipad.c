...
// current tap time in jiffies
static unsigned long touchdown_jiffies = INITIAL_JIFFIES;
// previous tap time in jiffies for double tap
static unsigned long touchdown_prev_jiffies = INITIAL_JIFFIES;
// last x and y
...
RESET:
    ...
    y_start = 0;
    tap_enabled = 1;
    touchdown_jiffies = INITIAL_JIFFIES;
    return;
}

/*!
 * @brief This is the work func which creates trackball events
 *
 * This function implements the mouse function.  This function
 * will process mouse movements in the X,Y plane and produce the 
...
static void minipad_process_mouse(struct minipad_data *ts,unsigned int x,unsigned int y, unsigned int touch)
{
    int x_temp=0;
    int y_temp=0;
    int mouse_direction = MINIPAD_KEY_NULL;
    long comp_jiffies = 0;
    ...
    if(!x_start && !y_start && touch)
    {
        ...
        y_start = y;
        tap_enabled = 1;
        touchdown_jiffies = current_jiffies;
        minipad_log("mouse: <<< Touchdown: x=%4i, y=%4i, t=(%lu)\n", x, y, touchdown_jiffies);
        return;
    }

    // calc how far finger moved in x and y and get moving direction
    x_temp = abs(x - x_start);
    y_temp = abs(y - y_start);
    if(x_temp >= y_temp)
    {
        if(x > x_start)
        {
            mouse_direction = MINIPAD_KEY_RIGHT;
            minipad_log("mouse: R:x= %4i -> %4i = %4i\n",x_start ,x, x_temp);
        }
        else
        {
            mouse_direction = MINIPAD_KEY_LEFT;
    ...
    else
    {
        if(y > y_start)
        {
            mouse_direction = MINIPAD_KEY_DOWN;
            minipad_log("mouse: D:y= %4i -> %4i = %4i\n",y_start ,y, y_temp);
        }
        else
        {
            mouse_direction = MINIPAD_KEY_UP;
    ...
    if(!touch)
    {
        ...
        if(tap_enabled)
        {
            ...
            comp_jiffies = (long)current_jiffies - (long)touchdown_jiffies;
            //minipad_log("mouse: >>> Tap time (%lu)->(%lu)=(%lu)\n", touchdown_jiffies, current_jiffies, comp_jiffies);
            if(
               (comp_jiffies >= MINIPAD_TAP_TIME_MIN_TH) && (comp_jiffies <= MINIPAD_TAP_TIME_MAX_TH) && 
            ...
            {
                // check double tap?
                comp_jiffies = (long)touchdown_jiffies - (long)touchdown_prev_jiffies;
                if((comp_jiffies >= MINIPAD_DOUBLE_TAP_TIME_MIN_TH) && (comp_jiffies <= MINIPAD_DOUBLE_TAP_TIME_MAX_TH))
                {
                    // yes, this is the second tap of double tap
                    /* Touch event happened within the alotted time so send the select */
                    minipad_log("mouse: Double tap -> press CENTER\n");
                    input_report_key(ts->input_dev, MINIPAD_KEY_CENTER, MINIPAD_KEY_PRESS);
                    input_report_key(ts->input_dev, MINIPAD_KEY_CENTER, MINIPAD_KEY_RELEASE);
                    touchdown_prev_jiffies = INITIAL_JIFFIES;
                }
                else
                {
                    // no, this is the first tap
                    touchdown_prev_jiffies = touchdown_jiffies;
                }
            }
        }
        // bye
    ...
    else
    {
        ...
        case Q5199225K03_10BIT_TSP_MSG:
            ...
            if (buf[Q5199225K03_MSG_OBJ_FLAG_BYTE] == 0x41)
            {
                ...
                // check flip status, ignore touch event and exit if flip is closed
                if(ts->logic_params.check_flip)
                {
                    ret = gpio_get_value(MINIPAD_FLIP_GPIO);
                    if(!ret)
                    {
                        minipad_log("minipad_work_func: ignoring - flip closed\n");
                        goto EXIT;
                    }
                }
        ...
        case Q5199225K03_STATUS_MSG:
            ...
            {
                if(ts->logic_params.check_overrun)
                {
                ...
                {
                    minipad_log_critical("minipad_work_func: WARNING overrun error\n");
                }
            }
            break;
...
static int minipad_reg_read(struct i2c_client *client , int reg )
{
    ...
    {
        // force read if isr failed to get data
        rc = i2c_master_recv(client, data, 6);
        if (rc > 0)
        {
            rc = data[2+(reg%4)];
        }
...
static void minipad_logic_params_init(int version)
{
    if(version == 1) //zeppelin
    {
        ...
        ts->logic_params.check_overrun = true;
    ...
    else //motus/default
    {
        ...
        ts->logic_params.check_overrun = false;
...
static int minipad_ioctl(struct inode *node, struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case MINIPAD_IOCTL_SET_BOOTLOADER_MODE:
        ...
        if ( ts->mode != MINIPAD_MODE_BOOTLOADER )
        {
            ...
            minipad_reg_write (ts->client, Q5199225K03_RESET_ADDR,
                 1, (char *)&regValue);
            clk_busy_wait(Q5199225K03_WAIT*100);
    ...
    case MINIPAD_IOCTL_GET_FLIPSTATUS:
        rc = gpio_get_value(MINIPAD_FLIP_GPIO);
        minipad_log("minipad_ioctl: get_flipstatus =%d\n", rc);
        break;
