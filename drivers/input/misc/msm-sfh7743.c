...
static long  sfh7743_fops_ioctl (struct file *, unsigned int, unsigned long);
...
static struct file_operations sfh7743_fops =
{
   ...
   .unlocked_ioctl   = sfh7743_fops_ioctl,
 ...
 */
static long sfh7743_fops_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
   /** \var error
...
#ifdef CONFIG_MACH_MOT
	irq_set_irq_wake(sfh7743_info.irq, 1);
#else
...
#ifdef CONFIG_MACH_MOT
	irq_set_irq_wake(sfh7743_info.irq, 0);
#endif
