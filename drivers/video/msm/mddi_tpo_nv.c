...
#include "../../../arch/arm/mach-msm/board-mot.h" //GPIO/I2C addresses
...
static struct init_table const tpo_nv_plat_init[] =
{
   ...
   { MM_SET_ADDR_MODE_PARAM2, 1   }, //Enable HSM
   { TABLE_END,            0x0000 }, /* terminate table */
};
...
static void msm_fb_up(struct platform_device *pdev)
{
   ...
   if (cache_pdev != pdev)
   {
      ...
      if (!mfd)
      {
         printk(KERN_ERR "TPO NV - Bad sem pointer!!\n");
         return;
      }
      cache_pdev = pdev;
   }
...
static void msm_fb_down(struct platform_device *pdev)
{
   static struct platform_device *cache_pdev = NULL;
   static struct msm_fb_data_type *mfd = NULL;
...
static struct platform_driver mddi_tpo_nv_driver __refdata = {
