...
static struct msm_pm_time_stats {
	...
	[MSM_PM_STAT_IDLE_STANDALONE_POWER_COLLAPSE].name = "idle-standalone-power-collapse",
	[MSM_PM_STAT_IDLE_STANDALONE_POWER_COLLAPSE].first_bucket_time =
		CONFIG_MSM_IDLE_STATS_FIRST_BUCKET,

	[MSM_PM_STAT_IDLE_FAILED_STANDALONE_POWER_COLLAPSE].name = "idle-failed-standalone-power-collapse",
...
void arch_idle(void)
{
	...
	if (sleep_time < msm_pm_idle_sleep_min_time || !allow_sleep) {
		...
		case MSM_PM_SLEEP_MODE_APPS_SLEEP:
			if (ret)
				exit_stat = MSM_PM_STAT_FAILED_SUSPEND;
			else
...
{
	return msm_pm_modes[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_NO_XO_SHUTDOWN].latency - 1;
}
