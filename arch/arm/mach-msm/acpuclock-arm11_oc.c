...
unsigned long acpuclk_power_collapse(void) {
	int ret = acpuclk_get_rate(0);
	acpuclk_set_rate(0, drv_state.power_collapse_khz, SETRATE_PC);
	return ret * 1000;
}

unsigned long acpuclk_wait_for_irq(void) {
	int ret = acpuclk_get_rate(0);
	acpuclk_set_rate(0, drv_state.wait_for_irq_khz, SETRATE_SWFI);
	return ret * 1000;
}
...
void __init msm_acpu_clock_init(struct msm_acpu_clock_platform_data *clkdata)
{
	...
	drv_state.acpu_switch_time_us = 50 /*clkdata->acpu_switch_time_us*/;
